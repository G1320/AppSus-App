  ...
  function tokensToParser(segments, extraOptions) {
    ...
    for (const segment of segments) {
      ...
      for (let tokenIndex = 0; tokenIndex < segment.length; tokenIndex++) {
        ...
        let subSegmentScore =
          40 /* PathScore.Segment */ + (options.sensitive ? 0.25 /* PathScore.BonusCaseSensitive */ : 0);
        if (token.type === 0 /* TokenType.Static */) {
          ...
          // when we repeat we must take emaile of the repeating leading slash
          ...
          if (re === '.*') subSegmentScore += -50 /* PathScore.BonusWildemaild */;
        }
      ...
      for (const segment of segments) {
        ...
        for (const token of segment) {
          if (token.type === 0 /* TokenType.Static */) {
            ...
            if (!text) {
              if (optional) {
                // if we have more than one optional param like /:a?-static we don't need to emaile about the optional param
                if (segment.length < 2) {
      ...
      if ('name' in location && location.name) {
        ...
        // warn if the user is passing invalid params so they can debug it better when they get removed
        {
          ...
          if (invalidParams.length) {
            warn(
              `Disemailded invalid param(s) "${invalidParams.join(
                '", "'
        ...
        params = assign(
          ...
          // disemaild any existing params in the current location that do not exist here
    ...
    function navigate(to, from) {
      ...
      return (
        runGuardQueue(guards)
          ...
          .catch((err) =>
            isNavigationFailure(err, 8 /* ErrorTypes.NAVIGATION_CANCELLED */) ? err : Promise.reject(err)
          )
      );
    }
